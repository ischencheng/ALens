...
def lemma_dicter(lemma_file):
	lemma_list = file((lemma_file),'rU').read().split("\n")
	lemma_dict = {}
...
def call_stan_corenlp_pos(class_path, file_list, output_folder, memory, nthreads, system, dataQueue,root, parse_type = ""): #for CoreNLP 3.5.1 (most recent compatible version)
	...
	if system == "M" or system == "L":
		print class_path
		call_parser = "java -cp "+ class_path +"stanford-corenlp-3.5.1.jar:stanford-corenlp-3.5.1-sources.jar:stanford-corenlp-3.5.1-models.jar:xom.jar: -Xmx" + memory + "g edu.stanford.nlp.pipeline.StanfordCoreNLP -threads "+ nthreads + " -annotators tokenize,ssplit,pos,lemma"+parse_type+" -filelist " + file_list + " -outputDirectory "+ output_folder + " -outputFormat xml"
	#windows call:
	elif system == "W":
...
def simple_list_dict_builder(database_file, delimiter):
	...
	data_file = file(database_file, "rU").read().lower().split("\n")
...
def simple_proportion(target_text,ref_text,type,index_list = None,index_name= None,header_list = None): #each text is a list
...
def syn_overlap(target_text, reference_text, syn_dict,index_list = None,index_name= None,header_list = None, list = "no"):
	target = []
	reference = []
	...
	reference = set([item for sublist in reference for item in sublist]) #flattens list
...
def keyness(target_list, frequency_list_dict, top_perc = None): #note that frequency_list_dict should be normed
	list = []
